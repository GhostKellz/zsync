...
pub fn allocateStack(allocator: std.mem.Allocator) ![]align(16) u8 {
    ...
    if (comptime @hasDecl(std.posix, "mprotect")) {
        ...
        const page_size = 4096; // Standard page size
        if (stack.len >= page_size * 2) {
            const aligned_page = @as([*]align(4096) u8, @alignCast(stack.ptr))[0..page_size];
            std.posix.mprotect(aligned_page, std.posix.PROT.NONE) catch {};
        }
    }
...
pub fn deallocateStack(allocator: std.mem.Allocator, stack: []u8) void {
    ...
    if (comptime @hasDecl(std.posix, "mprotect")) {
        ...
        if (stack.len >= page_size * 2) {
            const aligned_page = @as([*]align(4096) u8, @alignCast(stack.ptr))[0..page_size];
            std.posix.mprotect(aligned_page, std.posix.PROT.READ | std.posix.PROT.WRITE) catch {};
        }
    }
...
pub fn readTimeStampCounter() u64 {
    ...
    asm volatile ("mrs %[time], cntvct_el0"
        : [time] "=r" (time),
    );
