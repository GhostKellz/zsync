...
pub const HybridMetrics = struct {
    ...
    pub fn recordSwitch(self: *Self, to_model: ExecutionModel) void {
        ...
        self.last_switch_time.store(@intCast(blk: { const ts = compat.clock_gettime(std.os.linux.CLOCK.REALTIME) catch unreachable; break :blk @intCast(@divTrunc((@as(i128, ts.sec) * std.time.ns_per_s + ts.nsec), std.time.ns_per_ms)); }), .release);
    ...
    pub fn canSwitch(self: *Self, cooldown_ms: u64) bool {
        const current_time = @as(u64, @intCast(blk: { const ts = compat.clock_gettime(std.os.linux.CLOCK.REALTIME) catch unreachable; break :blk @intCast(@divTrunc((@as(i128, ts.sec) * std.time.ns_per_s + ts.nsec), std.time.ns_per_ms)); }));
