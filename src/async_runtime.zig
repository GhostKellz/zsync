...
pub const Sleep = struct {
    ...
    pub fn init(duration_ms: u64) Self {
        return Self{
            ...
            .start_time = @intCast(blk: { const ts = compat.clock_gettime(std.os.linux.CLOCK.REALTIME) catch unreachable; break :blk @intCast(@divTrunc((@as(i128, ts.sec) * std.time.ns_per_s + ts.nsec), std.time.ns_per_ms)); }),
        };
    }
