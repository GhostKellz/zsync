...
fn performFileWrite(context: *WriteContext) !void {
    ...
    if (context.options.atomic) {
        ...
        const temp_path = try std.fmt.allocPrint(
            ...
            "{s}.tmp.{d}",
            .{ context.path, blk: { const ts = compat.clock_gettime(std.os.linux.CLOCK.REALTIME) catch unreachable; break :blk @intCast(@divTrunc((@as(i128, ts.sec) * std.time.ns_per_s + ts.nsec), std.time.ns_per_ms)); } }
        );
