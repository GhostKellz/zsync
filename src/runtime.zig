...
pub fn formatError(err: RuntimeError) []const u8 {
    return switch (err) {
        error.ThreadPoolExhausted => "Thread pool has no available workers. Consider increasing thread_pool_threads in Config or reducing concurrent task load.",
        error.IoUringNotAvailable => "io_uring not available on this system. Linux kernel 5.1+ required. Consider using execution_model = .thread_pool instead.",
        error.InvalidThreadCount => "Invalid thread count specified. Must be between 1 and 128. Use 0 for auto-detection based on CPU count.",
        error.BufferSizeTooSmall => "Buffer size too small. Minimum 1024 bytes recommended. Increase Config.buffer_size for better performance.",
        error.GreenThreadsNotSupported => "Green threads not supported on this platform. Only available on Linux with io_uring. Use execution_model = .thread_pool instead.",
        error.ConfigurationError => "Invalid configuration detected. Run Config.validate() for detailed error information.",
        error.PlatformUnsupported => "This execution model is not supported on your platform. Use ExecutionModel.detect() for optimal model.",
        error.AlreadyRunning => "Runtime is already running. Cannot start multiple instances simultaneously.",
        error.RuntimeShutdown => "Runtime has been shutdown. Create a new Runtime instance to continue.",
        error.TaskQueueFull => "Task queue is full. Too many pending tasks. Wait for some to complete or increase queue_size.",
        error.FutureAlreadyAwaited => "Future has already been awaited. Futures can only be awaited once.",
        error.TimeoutExpired => "Operation timed out. Increase timeout duration or check if operation is stalled.",
        else => @errorName(err),
    };
}
