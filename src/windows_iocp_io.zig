...
pub const WindowsIocpIo = struct {
    ...
    fn connect(context: *anyopaque, fd: posix.fd_t, address: *const posix.sockaddr) IoError!Future {
        _ = context;
        _ = fd;
        _ = address;
        return IoError.NotSupported;
...
const WindowsAsyncOp = struct {
    overlapped: if (builtin.os.tag == .windows) windows_platform.OVERLAPPED else void,
    ...
    pub fn initRead(buffer: []u8, completion_key: usize) WindowsAsyncOp {
        ...
        return WindowsAsyncOp{
            .overlapped = std.mem.zeroes(windows_platform.OVERLAPPED),
            .operation_type = .read,
            .buffer = buffer,
            .completion_key = completion_key,
        };
