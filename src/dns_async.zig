...
pub const AsyncDnsResolver = struct {
    ...
    fn cacheResponse(self: *Self, cache_key: []const u8, response: DnsResponse, ttl: u32) !void {
        ...
        // Check cache size and evict if necessary. If eviction does not free a slot,
        // fail closed instead of silently exceeding the configured limit.
        if (self.cache.count() >= self.max_cache_entries) {
            try self.evictOldestEntry();
            if (self.cache.count() >= self.max_cache_entries) {
                return DnsError.CacheError;
            }
        }
    ...
    fn parseDnsName(self: *Self, buffer: []const u8, offset: *usize) ![]u8 {
        ...
        while (current_offset < buffer.len) {
            ...
            if (len == 0) {
                ...
                // Compression pointer
                if (current_offset + 1 >= buffer.len) return DnsError.InvalidResponse;
                if (!jumped) offset.* = current_offset + 2;

                jump_count += 1;
                if (jump_count > max_dns_name_jumps) return DnsError.InvalidResponse;

                const pointer = (@as(usize, len & 0x3F) << 8) | @as(usize, buffer[current_offset + 1]);
                if (pointer >= buffer.len) return DnsError.InvalidResponse;
                current_offset = pointer;
                jumped = true;
            } else {
                if (len > 63) return DnsError.InvalidResponse;
                current_offset += 1;
        ...
        if (current_offset >= buffer.len and (buffer.len == 0 or buffer[buffer.len - 1] != 0)) {
            return DnsError.InvalidResponse;
        }
...
test "DNS parser rejects truncated compression pointer" {
    var resolver = AsyncDnsResolver.init(testing.allocator);
    defer resolver.deinit();

    var offset: usize = 0;
    const packet = [_]u8{0xC0};
    try testing.expectError(DnsError.InvalidResponse, resolver.parseDnsName(&packet, &offset));
}

test "DNS parser rejects compression loop" {
    var resolver = AsyncDnsResolver.init(testing.allocator);
    defer resolver.deinit();
...
test "DNS parser rejects oversized label" {
    var resolver = AsyncDnsResolver.init(testing.allocator);
    defer resolver.deinit();

    var packet: [66]u8 = undefined;
    packet[0] = 64;
    @memset(packet[1..65], 'a');
    packet[65] = 0;
