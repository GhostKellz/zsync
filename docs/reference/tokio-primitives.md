...
|-----------|-------------|--------|
| `tokio_sync.Notify` | Task notification (wait/notifyOne/notifyAll) | Full |
| `tokio_sync.OnceCell` | Thread-safe lazy initialization | Full |
| `tokio_sync.CancellationToken` | Hierarchical cancellation with child tokens | Full |
| `time.Interval` | Repeating timer | Full |

### Functional

| Primitive | Description | Status |
|-----------|-------------|--------|
| `task.JoinSet` | Concurrent task management | Thread-based concurrency with `joinNext()` |
| `tokio_sync.BroadcastChannel` | Multi-producer multi-consumer pub/sub | Capacity limits, lag tracking, blocking recv |
| `tokio_sync.WatchChannel` | Single-value observer pattern | Watcher with `changed()` blocking wait |
| `task.Nursery` | Structured concurrency | Full with error propagation |
| `channel.mpsc` | Bounded/unbounded channels | Sender/receiver facade over the ring buffer implementation |
...
### Runtime-Integrated Sync Primitives

These use `std.Io` wait/futex/condition primitives and expose cancellation or
timeout points where the standard library supports them.

| Primitive | Description | Notes |
|-----------|-------------|-------|
| `IoSemaphore` | Counting semaphore with max bounds | `acquire`, `acquireTimeout`, `tryAcquire` |
| `IoMutex` | Mutual exclusion lock | Cancelable `lock(io)` plus `tryLock` |
| `IoRwLock` | Reader-writer lock | Cancelable read/write locks |
| `IoWaitGroup` | Wait for group completion | Cancelable and timeout waits |

### Compatibility

| Primitive | Description | Notes |
|-----------|-------------|-------|
| `compat.RuntimeBuilder` | Fluent runtime configuration | Advisory under `std.Io.Threaded`; root alias retained |
| Root aliases | `JoinSet`, `BroadcastChannel`, `WatchChannel`, `Notify`, `OnceCell`, `CancellationToken` | Retained for older consumers; new code should use module namespaces |
...
| Runtime-integrated waits for `Barrier`/`Latch` | Yield to runtime instead of blocking | v0.9.0+ |
...
var set = zsync.task.JoinSet(u32).init(allocator);
defer set.deinit();
...
```zig
var broadcast = zsync.tokio_sync.BroadcastChannel(Event).initWithCapacity(allocator, 32);
...
if (zsync.tokio_sync.BroadcastChannel(Event).recv(sub1)) |event| {
    handleEvent(event);
}
...
```zig
var watch = zsync.tokio_sync.WatchChannel(Config).init(defaultConfig);
